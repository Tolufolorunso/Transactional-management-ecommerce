...
async function seedDB() {
  ...
  //--------------------Briefcases
  const briefcases_titles = ['Aluminium Metal Suitcase', 'Black Leather Durable Suitcase']
  ...
  async function seedProducts(titlesArr, imgsArr, categStr) {
    try {
      ...
      const categ = ['female', 'male', 'ladies']
      for (let i = 0; i < titlesArr.length; i++) {
        let prod = new Product({
          productCode: faker.helpers.replaceSymbolWithNumber('####-##########'),
          title: titlesArr[i],
          images: [
            { url: imgsArr[faker.datatype.number({ min: 1, max: 3 })], filename:titlesArr[i] },
            { url: imgsArr[faker.datatype.number({ min: 1, max: 3 })], filename:titlesArr[i] },
            { url: imgsArr[faker.datatype.number({ min: 1, max: 3 })], filename:titlesArr[i] }],
          ...
          available: true,
          userID: '6161b627a2a39927ce064e48',
